...
- **Generation**: `supabase/functions/outfit-tryon` (Deno). Uses `npm:@gradio/client` to call **CatVTON** (`zhengchong/CatVTON`, Space endpoint `/submit_function`) with `HF_TOKEN` as the Space auth token, falling back to **IDM-VTON** (`yisol/IDM-VTON`, endpoint `/tryon`) if CatVTON errors, one retry each. Image blobs passed to `predict()` **must** be wrapped in `handle_file()` from `@gradio/client` — passing raw `Blob`s directly silently produces wrong/garbage results (this was an actual bug found in production: a real request returned an unrelated denim jacket instead of the user's own garment, because unwrapped blobs weren't being recognized as valid file inputs by the Space). Fixed 2026-07-21.
- **Model limitation (permanent, by design)**: both CatVTON and IDM-VTON are garment try-on models — `cloth_type` only supports `upper` / `lower` / `overall`. **Neither supports shoes or accessories**, confirmed from each Space's live API config, not assumed. The Edge Function filters steps down to `upper`/`lower` only and always skips anything else (logged, not an error). This is not a bug to fix — don't add a shoes/accessory step without switching to a completely different model.
